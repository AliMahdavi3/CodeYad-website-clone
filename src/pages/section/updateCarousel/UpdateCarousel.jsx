...
import { updateCourses } from '../../../core/updateCourses';

const UpdateCarousel = () => {
    const [items, setItems] = useState(updateCourses);
    const responsive = {
        superLargeDesktop: {
  ...
  return (
    ...
    responsive={responsive} autoPlay={true} infinite={true} customTransition="all 3s" autoPlaySpeed={3000}
     className='rounded-5 w-100 mt-5 '>
        {
            items.map((item)=>{
                return (
                    <div className='card mx-3 my-3 rounded-5 border-0 text-end' key={item.id}>
